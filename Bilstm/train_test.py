...
'''
This python scrip defines train and evaluate method. Beam search is used to improve the search efficiency 
and output quality of sequence generation models by maintaining a set of top-scoring candidate sequences 
...
def translate(src: str) -> str:
    ...
    with torch.no_grad():
        attention_weights = torch.stack(attention_weights).squeeze().cpu().numpy()
